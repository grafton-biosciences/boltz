...
class ProteinProteinAffinityCropper(Cropper):
    ...
    def _detect_interface_residues(
        self, receptor_tokens: np.ndarray, binder_tokens: np.ndarray
        ...
        dist_matrix = np.sqrt(
            np.sum(
                (receptor_coords[:, None, :] - binder_coords[None, :, :]) ** 2, axis=-1
            )
        )
    ...
    def crop(
        ...
        if len(receptor_interface_idx) < self.min_interface_residues:
            ...
            receptor_interface_idx = np.argsort(min_dists)[
                : self.min_interface_residues
            ]
        ...
        for token in receptor_interface_tokens:
            ...
            new_atoms = np.sum(
                new_tokens[np.isin(new_tokens["token_idx"], list(new_indices))][
                    "atom_num"
                ]
            )
    ...
    def _get_token_neighborhood(
        self, query_token: np.ndarray, chain_tokens: np.ndarray
    ) -> np.ndarray:
        ...
        while tokens.size < self.neighborhood_size and (
            current_min > min_idx or current_max < max_idx
        ):
