...
class EnsembleProteinAffinityModule:
    ...
    def _select_ensemble_residues(
        ...
        if num_binder_residues == 0:
            raise ValueError(
                "No binder residues found for ensemble affinity prediction"
            )
        ...
        if (
            self.ensemble_sampling_strategy == "all"
            or num_binder_residues <= ensemble_size
        ):
            ...
            token_coords = torch.bmm(token_to_rep_atom.float(), x_pred)[
                0
            ]  # [tokens, 3]
            ...
            _, closest_indices = torch.topk(
                min_distances, k=ensemble_size, largest=False
            )
            return binder_indices[closest_indices], min_distances[closest_indices]

        else:
            raise ValueError(
                f"Unknown ensemble sampling strategy: {self.ensemble_sampling_strategy}"
            )

    def _compute_ensemble_weights(
        self, min_distances: torch.Tensor, sigma: float = 4.0
    ):
        """
    ...
    def _retokenize_residue_as_atoms(self, tokenized_data, residue_token_idx):
        ...
        res_name = (
            residue_token["res_name"]
            if "res_name" in residue_token.dtype.names
            else None
        )
        ...
        if res_name:
            ...
            if ccd_mol is not None:
                try:
                    ...
                    parsed_ccd_residue = parse_ccd_residue(
                        res_name, ccd_mol, res_idx, drop_leaving_atoms=True
                    )
        ...
        for i, atom in enumerate(atom_data):
            ...
            element = (
                atom["element"] if "element" in atom.dtype.names else 6
            )  # Default to carbon
        ...
        if parsed_ccd_residue is not None and parsed_ccd_residue.bonds:
            ...
            for bond in parsed_ccd_residue.bonds:
                ...
                if bond.atom_1 < len(parsed_ccd_residue.atoms) and bond.atom_2 < len(
                    parsed_ccd_residue.atoms
                ):
                    ...
                    if (
                        atom1_name in atom_name_to_token_idx
                        and atom2_name in atom_name_to_token_idx
                    ):
    ...
    def _create_enhanced_bonds(
        self, tokenized_data, atomic_tokens, enhanced_bonds, residue_token_idx
    ):
        """
        ...
        original_token_idx = tokenized_data.tokens[residue_token_idx][
            0
        ]  # token_idx field

        # Filter out bonds involving the original residue token
        for bond in original_bonds:
            token1, token2, bond_type = (
                bond  # TokenBondV2 format (token1, token2, bond_type)
            )
    ...
    def _create_retokenized_structure(self, tokenized_data, residue_token_idx):
        ...
        atomic_tokens, enhanced_bonds = self._retokenize_residue_as_atoms(
            tokenized_data, residue_token_idx
        )
        ...
        if original_remaining_start < num_original:
            ...
            remaining_tokens = (
                original_tokens[original_remaining_start:].copy().tolist()
            )
    ...
    def _create_atomic_features(self, feats, residue_token_idx):
        ...
        atomic_tokenized = self._create_retokenized_structure(
            feats["featurizer_args"][0]["data"], residue_token_idx
        )

        num_atomic = len(atomic_tokenized.tokens) - len(
            feats["featurizer_args"][0]["data"].tokens
        )
        ...
        for key, value in atomic_feats.items():
            if isinstance(value, torch.Tensor):
                atomic_feats[key] = (
                    atomic_feats[key]
                    .unsqueeze(0)
                    .to(device=feats["affinity_token_mask"].device)
                )
    ...
    def _create_single_residue_features(
        self, feats: Dict[str, torch.Tensor], residue_idx: int
    ...
    def forward(
        ...
        ensemble_indices, min_distances = self._select_ensemble_residues(
            binder_indices, feats, x_pred, multiplicity
        )
        ...
        for residue_idx in ensemble_indices:
            ...
            if self.atomic_affinity:
                ...
                single_residue_feats = self._create_single_residue_features(
                    feats, residue_idx.item()
                )
            ...
            lig_mask = single_residue_feats["affinity_token_mask"][0].to(
                torch.bool
        ...
        if len(ensemble_predictions) > 0:
            ...
            avg_prediction = (
                torch.stack(ensemble_predictions) / len(ensemble_predictions)
            ).sum(dim=0)
            avg_probability = (
                torch.stack(ensemble_probabilities) / len(ensemble_predictions)
            ).sum(dim=0)
            # Convert averaged probability back to logits for consistency
            avg_logits = torch.logit(
                torch.clamp(avg_probability, min=1e-7, max=1 - 1e-7)
            )
            ...
            binder_affinity_values = torch.stack(
                ensemble_predictions
            )  # [num_members, batch, 1]
            binder_affinity_probabilities = torch.stack(
                ensemble_probabilities
            )  # [num_members, batch, 1]
            ...
            return {
                ...
                "ensemble_size": torch.tensor(
                    len(ensemble_predictions), dtype=torch.long
                ),
            ...
            return {
                ...
                "affinity_probability_binary": torch.zeros(
                ...
                "binder_affinity_values": torch.zeros(
                    (0, batch_size, 1), device=device
                ),
                "binder_affinity_probabilities": torch.zeros(
                    (0, batch_size, 1), device=device
                ),
