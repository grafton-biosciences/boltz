...
if affinity_state_dict:
    missing_keys, unexpected_keys = affinity_module1.load_state_dict(
        affinity_state_dict, strict=False
    )
...
def create_batch(sample_dict, batch_size):
    ...
    batch = {}
    batch["s_inputs_affinity"] = (
        sample_dict["s_inputs_affinity"].repeat(batch_size, 1, 1).to(device)
    )
    batch["z_affinity"] = (
        sample_dict["z_affinity"].repeat(batch_size, 1, 1, 1).to(device)
    )
    batch["coords_affinity"] = (
        sample_dict["coords_affinity"].repeat(batch_size, 1, 1, 1).to(device)
    )
...
with torch.no_grad():
    ...
    print(
        f"  Predicted affinity value: {final_output['affinity_pred_value'].item():.4f}"
    )
    print(
        f"  Predicted binary probability: {final_output['affinity_probability_binary'].item():.4f}"
    )
